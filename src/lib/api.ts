...
export class ApiClient {
  ...
  // ===== Month Themes =====
  async fetchMonthThemesForYear(year: number): Promise<import('../types').MonthTheme[]> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    ...
    if (error) throw error;
    return (data || []) as import('../types').MonthTheme[];
  }

  async fetchMonthTheme(year: number, month: number): Promise<import('../types').MonthTheme | null> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    ...
    if (error) throw error;
    return data as import('../types').MonthTheme | null;
  }

  async upsertMonthTheme(input: import('../types').MonthThemeInput): Promise<import('../types').MonthTheme> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    
    // Validate focus_areas (max 3) and non_focus_areas (max 2)
    const focusAreas = (input.focus_areas || []).slice(0, 3);
    const nonFocusAreas = (input.non_focus_areas || []).slice(0, 2);
  ...
  async deleteMonthTheme(year: number, month: number): Promise<void> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
  ...
  // ===== Year Themes =====
  async fetchYearTheme(year: number): Promise<import('../types').YearTheme | null> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    ...
    if (error) throw error;
    return data as import('../types').YearTheme | null;
  }

  async upsertYearTheme(input: import('../types').YearThemeInput): Promise<import('../types').YearTheme> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    ...
    if (error) throw error;
    return data as import('../types').YearTheme;
  }

  // ===== Year Goals =====
  async fetchYearGoals(year: number): Promise<import('../types').YearGoal[]> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    ...
    if (error) throw error;
    return (data || []) as import('../types').YearGoal[];
  }

  async createYearGoal(input: import('../types').YearGoalInput): Promise<import('../types').YearGoal> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        linked_habit_id: input.linked_habit_id ?? null,
  ...
  async updateYearGoal(id: string, patch: Partial<import('../types').YearGoalInput> & { 
  ...
  async deleteYearGoal(id: string): Promise<void> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
  ...
  async syncHabitToYearGoal(goalId: string): Promise<number> {
    const { supabase, isSupabaseConfigured } = await this.getSupabaseSafe();
    if (!isSupabaseConfigured || !supabase) throw new Error('Supabase not configured');
    
    // Get goal with habit link
    ...
    if (!goal || goal.data_source !== 'habit' || !goal.linked_habit_id) {
      return 0;
    }
    
    // Count habit completions for the year
    const yearStart = `${goal.year}-01-01`;
    const yearEnd = `${goal.year}-12-31`;
    
    const { count, error: countError } = await supabase
      .from('habit_entries')
      .select('date', { count: 'exact', head: true })
      .eq('habit_id', goal.linked_habit_id)
      .eq('is_done', true)
      .gte('date', yearStart)
      .lte('date', yearEnd);
    ...
    const completed = syncedCount >= goal.target_value;
    
    // Update current_value and completed status
    await this.updateYearGoal(goalId, { 
      current_value: syncedCount,
      completed,
